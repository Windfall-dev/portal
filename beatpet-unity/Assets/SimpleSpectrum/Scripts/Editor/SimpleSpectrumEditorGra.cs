...
[CustomEditor(typeof(SimpleSpectrumGra))]
[CanEditMultipleObjects]
public class SimpleSpectrumEditorGra : Editor {
    ...
    /*void NotifyPropertyChanged()
    {
        if (Application.isPlaying)
        {
            ...
            ((SimpleSpectrumGra)target).RebuildSpectrum();
        }
	}*/

    void OnEnable(){

        if(((SimpleSpectrumGra)target).muteGroup == null)
        {
            ((SimpleSpectrumGra)target).muteGroup = muteGroup;
        }
...
#if UNITY_WEBGL
        EditorGUILayout.LabelField("NOTE: SimpleSpectrumGra works with WebGL, but only under certain conditions. Check the docs!", new GUIStyle {wordWrap = true });
#endif
