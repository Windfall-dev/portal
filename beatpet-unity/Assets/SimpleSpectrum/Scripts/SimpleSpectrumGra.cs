...
public class SimpleSpectrumGra : MonoBehaviour
{
    ...
    */
    ...
    #region VARIABLES
    private Color[] cachedBarColors; // キャッシュされた色データ
    ...
    void Start()
    {
        cachedBarColors = new Color[barAmount];
    ...
    void InitializeColors()
    {
        ...
        //    {
        //        barMaterials[i].SetColor("_Color1", Color.white);
        //        barMaterials[i].SetColor("_Color2", Color.white);
        //    }
        //}
        ...
        for (int i = 0; i < barAmount; i++)
        {
            cachedBarColors[i] = GetColorForPosition(i, barAmount); // 各バーの色を事前に計算してキャッシュ
        }
        colorsInitialized = true;
    }

    private Color[] gradientColors = new Color[] {
        new Color(0.607f, 0.298f, 0.572f),//
        new Color(0.835f, 0.215f, 0.803f),
        new Color(0.866f, 0.227f, 0.596f),
        new Color(0.913f, 0.239f, 0.203f),
        new Color(0.949f, 0.674f, 0.282f),
        new Color(0.988f, 0.937f, 0.309f),
        new Color(0.807f, 0.992f, 0.321f),
        new Color(0.541f, 0.984f, 0.325f),//
        new Color(0.541f, 0.984f, 0.325f),//
        new Color(0.541f, 0.984f, 0.325f),//
        new Color(0.807f, 0.992f, 0.321f),
        new Color(0.988f, 0.937f, 0.309f),
    ...
    private Color GetColorForPosition(int index, int totalBars)
    {
        float position = (float)index / totalBars;
        int colorCount = gradientColors.Length;
        float scaledPosition = position * colorCount;

        int colorIndex = Mathf.FloorToInt(scaledPosition) % colorCount;
        float t = scaledPosition - Mathf.Floor(scaledPosition);

        Color color1 = gradientColors[colorIndex];
        Color color2 = gradientColors[(colorIndex + 1) % colorCount];
    ...
    void Update()
    {
        if (isEnabled)
        {
            ...
            for (int i = 0; i < bars.Length; i++)
            {
                ...
                // 色の設定を変更
                if (materialColourCanBeUsed && colorsInitialized)
                {
                    barMaterials[i].SetColor("_Color1", cachedBarColors[i]); // 色をキャッシュから設定
                }
